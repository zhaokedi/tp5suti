<?php
//获取所有门店数组键为id
function getshoparray(){
    $shop_list=db("shop")->select();
    $shop_arr=array();
    foreach($shop_list as $k=>$v){
        $shop_arr[$v['id']]=$v['title'];
    }
    return $shop_arr;
}
...
function getcityarray(){
    $city_list=db("city_list")->select();
    $city_srr=array();
    foreach($city_list as $k=>$v){
        $city_srr[$v['id']]=$v['name'];
    }
    return $city_srr;
}

//获取所有教练数组键为id
function getteacherarray($shopid=0){
    $map='1';
    if($shopid>0){
        $map.=" and shop_id ={$shopid}";
    }
    $city_list=db("teacher")->field('teacher_id,name')->where($map)->select();

    $city_srr=array();
    foreach($city_list as $k=>$v){
        $city_srr[$v['teacher_id']]=$v['name'];
    }
    return $city_srr;
}
//获取所有会员数组键为id
function getviparray(){

    $vip_list=db("vipcard")->field('id,title')->select();

    $vip_srr=array();
    foreach($vip_list as $k=>$v){
        $vip_srr[$v['id']]=$v['title'];
    }
    return $vip_srr;
}

/*阿里大于短信发送
 * @param $param  signname 签名 jsoncontent 模板变量 phone 手机号 tempcode模板代号 参数
 * @return boolean
 */
function smsAlidayu($param){
    ...
    $req->setSmsParam($param['jsoncontent']);
    $req->setRecNum($param['phone']);
    $req->setSmsTemplateCode($param['tempcode']);
    $resp = $c->execute($req);
    $sms_ok = '2';              //默认情况下不成功
 ...
 * @return string
 */
 function complateUrl($url, $domain = '') {
    ...
    if (false === stristr($url, $httpPre . "://") && !empty($url)) {//查找http:// 如果不存在
        ...
        $url = $httpPre . "://" . $domain . $port . '/' . $url; //拼接完整路径
    }
    return $url;
}
function fullimgpath($url, $domain = ''){
        return '/public'.$url;
    $httpPre = $_SERVER['SERVER_PORT'] == 443 ? 'https' : 'http';
    if (false === stristr($url, $httpPre . "://") && !empty($url)) {//查找http:// 如果不存在
        if (0 === strpos($url, '/')) {//查找首字母 如果存在
        ...
        if (empty($domain)) {
            $domain = config("SITE_DOMAIN") ? "www." . C("SITE_DOMAIN") : $_SERVER['HTTP_HOST'];
        }
        $port = $_SERVER['SERVER_PORT'] == "80" ? '' : ":" . $_SERVER['SERVER_PORT'];
        $url = $httpPre . "://" . $domain . $port . '/' . $url; //拼接完整路径
    }
...
//    return ROOT_PATH.'public'.$img;
}
//生成6位数开门密码--方法
function set_six_password(){
    $password = '';
    for ($i=0; $i<6; $i++){
        $temp_num = rand(0, 9);
        $password .= $temp_num;
    }
    $result = db('appointment')->where(array('status'=>1,'doornum'=>$password))->select();
    $result1 = db('onekey')->where(array('status'=>1,'doornum'=>$password))->select();
 ...
 * @return string
 */
function edit_point($openid,$point,$type=1){
    if($type==1){
        $data['point']=array('exp',"point+{$point}");
    }else{
        $data['point']=array('exp',"point-{$point}");
    }

    db('user')->where(array('openid'=>$openid))->save($data);
}
// 快速排序的算法
...
function getlevelname($point){
    $map['point_min']=array('lt',$point);
    $map['point_max']=array('gt',$point);
    $res=db('card_level')->where($map)->find();
...
function goods_thum_images($goods_id,$width,$height,$type=2){
    ...
    return __ROOT__."/Public".$goods['original_img'];
    if(empty($goods_id))
        return '';
    //判断缩略图是否存在
    ...
    $original_img = db('Goods')->where("id = $goods_id")->getField('original_img');
...
function refresh_stock($goods_id){
    ...
    db("Goods")->where("goods_id = $goods_id")->save(array('store_count'=>$store_count)); // 更新商品的总库存
}
...
function sendSMS($mobile, $code)
{
    ...
    if ($resp->result->success)
    {
        ...
        $data = db('sms_log')->where("code = '$code' and add_time > ".(time() - $sms_time_out))->find();
        // 没有就插入验证码,供验证用
...
function getCatGrandson ($cat_id)
{
    ...
    // 把整张表找出来
    $GLOBALS['category_id_arr'] = db('GoodsCategory')->getField('id,parent_id');
    // 先把所有儿子找出来
    $son_id_arr = db('GoodsCategory')->where("parent_id = $cat_id")->getField('id',true);
...
function getArticleCatGrandson ($cat_id)
{
    ...
    // 把整张表找出来
    $GLOBALS['cat_id_arr'] = db('ArticleCat')->getField('cat_id,parent_id');
    // 先把所有儿子找出来
    $son_id_arr = db('ArticleCat')->where("parent_id = $cat_id")->getField('cat_id',true);
    foreach($son_id_arr as $k => $v)
    {
...
{
    if(!empty($key))
        return  db("SpecGoodsPrice")->where("goods_id = $goods_id and `key` = '$key'")->getField('store_count');
    else
    ...
    if(empty($data)){
        ...
        if($result){
            ...
            foreach ($data as $k=>$v){
                ...
                if(!isset($temp[$k])){
                    db('config')->add($newArr);//新key数据插入数据库
                }else{
                    if($v!=$temp[$k])
                        db('config')->where("name='$k'")->save($newArr);//缓存key存在且值有变更新此项
                }
            }
            //更新后的数据库记录
...
function logOrder($order_id,$action_note,$status_desc,$user_id = 0)
{
    ...
    $order = db('morder')->where("order_id = $order_id")->find();
    $action_info = array(
        'order_id'        =>$order_id,
        'action_user'     =>$user_id,
    ...
    return db('order_action')->add($action_info);
}
...
{
    // 如果这笔订单已经处理过了
    $count = db('morder')->where("order_sn = '$order_sn' and pay_status = 0")->count();   // 看看有没已经处理过这笔订单  支付宝返回不重复处理操作
    if($count == 0) return false;
    // 找出对应的订单
    $order = db('morder')->where("order_sn = '$order_sn'")->find();
    // 修改支付状态  已支付
    db('morder')->where("order_sn = '$order_sn'")->save(array('pay_status'=>1,'pay_time'=>time()));
    // 减少对应商品的库存
    ...
    //分销设置
    db('rebate_log')->where("order_id = {$order['order_id']}")->save(array('status'=>1));
    // 成为分销商条件
...
//    $order = db('morder')->where(array('order_id'=>$id))->find();
...
//    //分销设置
//    db('rebate_log')->where("order_id = $id")->save(array('status'=>2));
...
function get_goods_promotion($goods_id,$user_id=0){
    $now = time();
    $goods = db('goods')->where("goods_id=$goods_id")->cache(true)->find();
    ...
    if($goods['prom_type'] == 1){//抢购
        $prominfo = db('flash_sale')->where($where)->cache(true)->find();
        if(!empty($prominfo)){
            if($prominfo['goods_num'] == $prominfo['buy_num']){
                ...
                $order_id_arr = db('morder')->where($where)->getField('order_id',true);
                if($order_id_arr){
                    $goods_num = db('order_goods')->where("prom_id=".$goods['prom_id']." and order_id in (".implode(',', $order_id_arr).")")->sum('goods_num');
    ...
    if($goods['prom_type']==2){//团购
        $prominfo = db('group_buy')->where($where)->cache(true)->find();
        if(!empty($prominfo)){
            if($prominfo['goods_num'] == $prominfo['buy_num']){
                $prom['is_end'] = 2;//已售馨
            }else{
    ...
    if($goods['prom_type'] == 3){//优惠促销
        ...
        $prominfo = db('prom_goods')->where($where)->cache(true)->find();
        if(!empty($prominfo)){
            if($prominfo['type'] == 0){
    ...
    if($prom['prom_id'] == 0){
        db('goods')->where("goods_id=$goods_id")->save($prom);
    }
    return $prom;
}
...
//    $goods_arr = db('goods')->where("goods_id in(".  implode(',',$goods_id_arr).")")->getField('goods_id,weight,market_price,is_free_shipping'); // 商品id 和重量对应的键值对
//
