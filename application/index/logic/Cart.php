...
class Cart extends Model
{
     ...
     */
    function addCart($goods_id,$goods_num,$user_id = 0)
    {       
        
        $goods = M('Goods')->where("id = $goods_id")->find(); // 找出这个商品
...
//            return array('status'=>-4,'msg'=>'商品库存不足','result'=>'');
        ...
        $where1 = " goods_id = $goods_id and  user_id = $user_id "; // 查询购物车是否已经存在这商品

        $catr_goods = M('Cart')->where($where1)->find(); // 查找购物车是否已经存在该商品

        $data = array(                    
                    'user_id'         => $user_id,   // 用户id
                    'goods_id'        => $goods_id,   // 商品id
                    'goods_name'        => $goods['goods_name'],   // 商品id
//                    'goods_sn'        => $goods['goods_sn'],   // 商品货号
                    'goods_price'     => $goods['goods_price'],  // 购买价
                    'member_goods_price' => $goods['goods_price'],  // 会员折扣价 默认为 购买价
       ...
       {            
            ...
            $cart_count = cart_goods_num($user_id); // 查找购物车数量
...
//        	$cartList[$k]['store_count']  = getGoodNum($val['goods_id'],''); // 最多可购买的库存数量
        	$cartList[$k]['goods_desc']  = htmlspecialchars_decode($goods['goods_desc']); // 最多可购买的库存数量
   ...
   return $freight;  
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price)
    {
        ...
        if($user['oauth']== 'weixin')
        {
            ...
            $jssdk = new \app\index\Logic\Jssdk($wx_user['appid'],$wx_user['appsecret']);
